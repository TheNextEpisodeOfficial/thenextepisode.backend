...
export class FavService {
   ...
   * @param favReqDto
   */
  async getFavList(favReqDto: FavReqDto): Promise<Pagination<FavResDto>> {
    try {
      ...
      const favRawMany = await fullQueryBuilder.getRawMany();
      // 결과를 DTO로 변환
      const favResults = plainToInstance(FavResDto, favRawMany);
      const favList = objectToCamel(favResults);
      return new Pagination<FavResDto>(favList, favIdsPaginated.meta);
    } catch (error) {
  ...
  async insertFavById(
    ...
    try {
      ...
      if (existItem) {
        return this.favRepository.update(
          { id: existItem.id },
          {
            delYn: "N",
          }
